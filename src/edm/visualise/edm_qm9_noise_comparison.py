...
# Sample molecules from **two** EDM diffusion models, predict chosen
# properties with one or more neural‑network property predictors and
# visualise how each model’s distribution compares with the ground‑truth
# QM9 distribution in side‑by‑side histograms (one row, two columns).
...
# class HistogramPlotter:
#     """Render QM9 vs two EDM distributions as a 1×2 subplot."""
...
#     def _common_edges(self, *arrays: np.ndarray) -> np.ndarray:
#         """Build shared bin edges spanning *all* arrays."""
#         _min = min(arr.min() for arr in arrays)
#         _max = max(arr.max() for arr in arrays)
#         return np.linspace(_min, _max, self.resolution + 1)
...
#         name_key: int,
#         label_data: str = "QM9",
#         edm_labels: Tuple[str, str] = ("EDM‑1", "EDM‑2"),
#     ) -> None:
#         """Save a figure with two subplots (QM9 vs each EDM)."""
#         title = property_names_safe[name_key]
#         v_data = prop_data.cpu().numpy()
...
#             ax.legend()
#         axes[0].set_ylabel("Frequency")
#         # fig.suptitle(f"{title} distribution – linear vs. cosine schedules", fontsize=13)
#         fig.tight_layout()
...
# def get_predictions_for_mols(
#     mols: Sequence, model: torch.nn.Module, cutoff: float
# ) -> torch.Tensor:
#     """Apply property predictor *model* to a list of mol objects."""
#     with torch.no_grad():
#         preds = [
#             model(
#                 FullyConnectedTransform(cutoff)(CenterOfMassTransform()(mol))
#             )
#             for mol in mols
#         ]
...
#     parser.add_argument("--resolution", type=int, default=50)
#     parser.add_argument(
#         "--save_folder_path", type=str, default="plots/histograms_two_edms"
#     )
#     parser.add_argument(
...
#         # ---- Predictions for each EDM sample set ---- #
#         preds_left = get_predictions_for_mols(
#             sample_sets[0], model, cfg["cutoff_data"]
#         )
#         preds_right = get_predictions_for_mols(
#             sample_sets[1], model, cfg["cutoff_data"]
#         )
